...
O principal componente das especificações OSGi é o Framework OSGi, ele fornece um ambiente padronizado onde as aplicações, denominadas \textit{bundles}, que definem os módulos do sistema, onde estes podem, em tempo de execução,  ser instalado, desinstalado, ativado ou desativado local ou remotamente.
...
Ela expande o dinamismo provido pelos textit{bundles} na camada de clico de vida, combinando este ao dinamismo provido por arquiteturas baseadas em serviço, onde serviços podem aparecer ou desaparecer a qualquer momento~\cite{hall2010osgi}.

\subsection{iPOJO}
\label{subsec:ipojo}
Outra tecnologia importante para a compreensão deste trabalho, é o framework iPOJO~\cite{ipojo}.

iPojo é um modelo de serviço orientado a componentes, que tem como principal objetivo simplificar o desenvolvimento de aplicações OSGi. É baseado no conceito de POJO (\textit{Plain Old Java Objects}), onde um POJO é basicamente um objeto java simples, que não possui nenhuma dependência do ambiente de execução, seguindo o princípio de que a simplicidade favorece a regularidade.
	
No iPojo, os diversos POJO's são encapsulados em containers, estes são responsáveis pelo gerenciamento das interações entre o POJO e o mundo externo ao container e podem ser facilmente estendidos através da utilização de \textit{handlers}~\cite{ipojo}.
...
A idéia por traz do iPojo se baseia nos conceitos de orientação a componentes, onde tempos a visão de modelo e instância de componente. O iPojo, o modelo de componente, chamado de \textit{component type} define propriedades do componente, classe de implementação e políticas de criação de instância. 
A instância herda todas as características do tipo do componente, porém, possui um nome exclusivo e um conjunto de configurações próprio do tipo chave-valor~\cite{ipojo}.

O iPojo é define uma ferramenta poderosa e extensível, que abstrai o uso de OSGi através do conceito de containers que controlam o acesso ao POJO, e de \textit{handlers} ``plugados'' aos containers, gerenciando a interação com entidades externas ou outros POJOs.
...
\begin{figure}[htp]
\centering
\includegraphics[width=8cm]{chapters/chapter2/pdca_cycle.png}
