...
No iPOJO, os diversos POJO's são encapsulados em containers, estes são responsáveis pelo gerenciamento das interações entre o POJO e o mundo externo ao container através da utilização de \textit{handlers}~\cite{ipojo}.
...
A idéia por traz do iPOJO se baseia nos conceitos de orientação a componentes, onde temos a visão de modelo e instância de componente. 

No iPOJO, o modelo de componente, chamado de \textit{component type}, define propriedades um conjunto de propriedades do componente, sua classe de implementação e políticas de criação de instância. 
A instância herda todas as características do \textit{component type} e pode ter um conjunto de propriedade próprias~\cite{ipojo}.

O iPojo é uma ferramenta poderosa e extensível, que abstrai o uso de OSGi através do conceito de containers que controlam o acesso ao POJO, e de \textit{handlers} ``plugados'' aos containers, gerenciando a interação com entidades externas ou outros POJOs.
